...
func _import(source_file, save_path, options, platform_variants, gen_files):
	print("Start Import")
	if YarnCompiler == null:
		printerr("Yarn compiler is not initalized yet, aborting import...")
		return ERR_PARSE_ERROR
	
	_import_yarn(source_file)
	
	var imported_marker_resource = Resource.new()
	imported_marker_resource.resource_name = ProjectSettings.globalize_path(source_file).get_file().trim_suffix(source_file.get_extension())
	imported_marker_resource.resource_path = "%s.%s" % [save_path, _get_save_extension()]
	
	var result = ResourceSaver.save(imported_marker_resource)
	if result != OK:
...
func _import_yarn(source_file: String):
	...
	if project_path == null:
		print("The yarn file %s is not currently associated with a Yarn Project." % source_file +
		...
		var project = ResourceLoader.load(project_path) as YarnProject
